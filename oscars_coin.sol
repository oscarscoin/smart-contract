...
contract OSCARSCOIN is ERC20, Ownable {
    ...
    constructor () ERC20("OSCARS COIN", "OSC") 
    {   
        address router;
        address pinkLock;
        ...
        maintenanceWallet = 0xFF07880F3Adb35b8D43842EC19e6C07b1c1DF38F;
        ...
        _mintOnce(owner(), 150e9 * (10 ** decimals()));
    ...
    function updateFees(uint256 _feeOnSell, uint256 _feeOnBuy, uint256 _feeOnTransfer) external onlyOwner {
        ...
        require(feeOnBuy <= 2, "CSLT: Total Fees cannot exceed the maximum");
        require(feeOnSell <= 2, "CSLT: Total Fees cannot exceed the maximum");
        require(feeOnTransfer <= 2, "CSLT: Total Fees cannot exceed the maximum");

        emit UpdateFees(feeOnSell, feeOnBuy);
    }

    event MaintenanceWalletChanged(address maintenanceWallet);

    function changeMaintenanceWallet(address _maintenanceWallet) external onlyOwner{
        require(_maintenanceWallet != address(0), "CSLT: Fee receiver cannot be the zero address");
        maintenanceWallet = _maintenanceWallet;

        emit MaintenanceWalletChanged(maintenanceWallet);
    }
    ...
    function swapAndSendFee(uint256 tokenAmount) private {
        ...
        payable(maintenanceWallet).sendValue(newBalance);
